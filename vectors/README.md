...
| `floatexp/` | `operations/params.json` only: each floatexp operation's inputs and result, mantissas as IEEE-754 bit patterns beside integer exponents | [floatexp.md](../spec/floatexp.md) |
| `t2-steps/` | `params.json` only: a crafted orbit (samples and small table), a pixel's start, and its count, final `z` and escape derivative `(dw, dE)` as bit patterns, run through the T2 loop | [deep-zoom.md](../spec/deep-zoom.md#t2-perturbation-past-1e290) |
