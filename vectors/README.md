...
|---|---|---|
| `mandelbrot/`, `julia/`, `burning-ship/`, `newton/` | `iterations.i32` (raw little-endian int32 escape counts, row-major); `roots.i32` for Newton; `orbit.c128` (raw complex128 reference orbit) for deep-zoom cases; `status.i32` (how each count was decided, 0–3) for status cases; `distance.f64` (raw little-endian float64 distance estimate, compared within the output's `relative_epsilon`) for distance cases; `bla_applications.i32` (each pixel's BLA skips) and, on some, `bla_table.f64` (the BLA table itself; `bla_table_x.f64` at T2) for BLA cases; `critical.c128` (the critical orbit rebased pixels restart on) for deep Julia cases; T2 cases (`t2-*`) ship no orbit, and pin its length, SHA-256 and small samples in `params.json` instead | [fractals.md](../spec/fractals.md), [deep-zoom.md](../spec/deep-zoom.md) |
...
| `nucleus/` | `params.json` only: an operation (`box`, `find`, or `snap`: the box, then Newton with its period and square), its inputs, and every field of the result — exact, but the sizes (IEEE-754 bit patterns, within the case's `relative_epsilon`) | [nucleus.md](../spec/nucleus.md) |
| `floatexp/` | `operations/params.json` and `division/params.json` only: each floatexp operation's inputs and result, mantissas as IEEE-754 bit patterns beside integer exponents | [floatexp.md](../spec/floatexp.md) |
| `t2-steps/` | `params.json` only: a crafted orbit (samples and small table), a pixel's start, and its count, final `z` and escape derivative `(dw, dE)` as bit patterns, run through the T2 loop; `bla-*` cases also build a T2 BLA table from the orbit (`"bla": {"dc_exponent": …}`) and expect its skip count and table words | [deep-zoom.md](../spec/deep-zoom.md#t2-perturbation-past-1e290) |
| `turns/` | `known-answers/params.json` only: `(k, n)` pairs and the bit patterns of `(cos 2πk/n, sin 2πk/n)` | [turns.md](../spec/turns.md) |
...
| `0.8.0` | BLA cases ([deep-zoom.md](../spec/deep-zoom.md#bla-bivariate-linear-approximation-mandelbrot-opt-in)): `"bla": true` in a Mandelbrot case's params and a `bla_applications` output (each pixel's number of skips, whose total must be positive). The table is built from the stored orbit, so a replay needs no bignum. |
| `0.9.0` | Discovery cases ([nucleus.md](../spec/nucleus.md)): the box period test and Newton's method for a nucleus in the orbit's fixed point, bit-exact to the printed center; the atom size and the framing's half-height within a relative `1e-12`, NaN equal to NaN; every key required for its operation, `null` written, never omitted. |
| `0.10.0` | T2 ([deep-zoom.md](../spec/deep-zoom.md#t2-perturbation-past-1e290)), zooms past `1e290` for Mandelbrot and Julia: [floatexp](../spec/floatexp.md) arithmetic and `pow10x`; `reference_small` / `critical_small` (the orbit's length, the SHA-256 of its little-endian complex128 samples, and its small samples as `[index, re mantissa bits, re exponent, im mantissa bits, im exponent]`), regenerated at the family's orbit zoom; Julia orbits at twice the frame's zoom at T1 and T2 alike (`t1-julia-i-precision-zoom100-12`); the crafted `t2-steps/` cases; navigation past zoom 290, its offsets rounded once to floatexp; BLA at T2 ([deep-zoom.md](../spec/deep-zoom.md#bla-at-t2)): `bla` Mandelbrot cases past zoom 290 with the top-level `dc_bound_exponent` (an integer `k`, or `null`), and the `bla_table_x` output (the T2 table: each double-double coefficient's `hi`, then each radius's mantissa and exponent, every NaN equal to every NaN); crafted `t2-steps/bla-*` cases, which add `"bla": {"dc_exponent": k or null}` and expect `applications` and `table` (the same words as IEEE-754 bit patterns). |
| `0.11.0` | BLA's coefficients in double-double ([deep-zoom.md](../spec/deep-zoom.md#bla-bivariate-linear-approximation-mandelbrot-opt-in), "Arithmetic"). Every case with a `bla` param at zoom 290 and below was rewritten under this version, and a runner refuses such a case below 0.11.0 (T2 `bla` cases, written at 0.10.0, always used these coefficients). What changed: the `bla_table` words of the two cases that carry one (`bla-11dim-offref-zoom30-24`, `bla-p1959-zoom30-16`), the counts of 9 chaotic pixels of `bla-p1959-zoom30-16`, and the distances of `bla-far-zoom170-16` by a few ulps (inside its epsilon); the other five cases differ only in `spec_version`. No case carries 0.8.0 any more. |
| `0.12.0` | Pinned turns ([turns.md](../spec/turns.md)): `cis_turns(k, n)` from integer arithmetic, no libm; Newton's roots of unity come from it (no Newton vector changed). |
