...
Java, and C engines all satisfy. Both heaton-life implementations replay these — Python in
`python/tests/test_mergelife.py`, .NET in
`dotnet/tests/HeatonLife.Core.Tests/MergeLifeUpstreamTests.cs` — so each is
byte-identical with the reference engines.
