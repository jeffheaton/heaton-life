...
namespace HeatonLife
{
    ...
    public sealed class GrayScott : IFloatFrameSource
    {
        ...
        public void SeedSpots(int spots, uint seed)
        {
            ...
            {
                FillHomogeneous();
                var replayRng = new Pcg32(s);
                for (int i = 0; i < spots; i++)
                {
                    int cx = (int)(replayRng.NextU32() % (uint)Width);
                    int cy = (int)(replayRng.NextU32() % (uint)Height);
                    SeedBox(cx, cy);
                }
                Generation = 0;
            });
        ...
        /// <summary>
        /// Restore a rectangle (inclusive corners) to the family blank — the
        /// substrate U = 1, V = 0, not zero (spec/patterns.md "Clear").
        /// </summary>
