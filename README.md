...
| Fractals | Newton's basins, and Mandelbrot, Julia, and Burning Ship with perturbation deep zoom far beyond float64 (Mandelbrot and Julia to 10⁹⁰⁰⁰) |
...
| [`dotnet/`](dotnet/) | C#/.NET implementation (netstandard2.1 core, library parity with Python; see Status) and its [development guide](dotnet/DEVELOPMENT.md) |
| [`docs/`](docs/) | The README gallery image (regenerated by `python/tools/gen_gallery.py`), the package icon, the Heaton Life app's user manuals, and privacy policies for the Heaton Life and Heaton Fractal apps |
...
playground support. The .NET port has library parity: every family, the colormap
pipeline, and the MergeLife evolver replay the shared vectors (bit-exact tiers exactly,
epsilon tiers within tolerance; MergeLife is byte-identical with the upstream engines).
Python-only: animation export (GIF and MP4, zoom-movie rendering included), JSON
round-trips of parameters and viewports, and the playground; the .NET port has the
zoom-movie plans and measured iteration budgets. Both ports carry a
[platform self-check](spec/self-check.md) that a host runs on its own runtime. See
...
(`heaton_life.__version__`, or the HeatonLife.Core package version).
