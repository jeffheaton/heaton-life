"""The featured MergeLife rules — spec/mergelife.md "Featured rules (the gallery)".

The MergeLife counterpart of the built-in pattern set: a rule here is 32 raw
octets with no mnemonic, so a curated, ordered catalog is the only practical way
into the family, and every implementation ships the same one. Apps surface it as
a rule gallery.
...
Reef — engineered by permuting Red World's octets, so it sits beside its parent.
Names and descriptions are authored from observed 128x128 soup behavior
(multi-seed runs to generation 640); they are presentation text, never part of a
wire format.

The order is part of the set. Pinned in tests/test_mergelife_gallery.py.
"""
...
class FeaturedRule:
    """One gallery entry: a canonical rule plus its presentation text."""
...
def _featured(rule: str, name: str, description: str) -> FeaturedRule:
    return FeaturedRule(rule, name, description)


MERGELIFE_GALLERY: tuple[FeaturedRule, ...] = (
    _featured(
